...
if __name__ == '__main__':
    ...
    for dataset_path in args.train_dataset:
        if args.dataset_type == 'voc':
            ...
            dataset = LapToolsDataset(args.root_folder, dataset_path, transform=train_transform,
                                      target_transform=target_transform)
            label_file = os.path.join(args.checkpoint_folder, "laptools-model-labels.txt")
            store_labels(label_file, dataset.class_names)
            num_classes = len(dataset.class_names)
    ...
    if args.dataset_type == "voc":
        ...
        val_dataset = LapToolsDataset(args.root_folder, args.validation_dataset, transform=test_transform,
                                 target_transform=target_transform, is_test=True)
    ...
    if args.resume is None:
        ...
        last_epoch = int(re.search('.*[Epoch\-]([0-9]{1,4})[\-Loss]', args.resume).group(1))
    ...
    for epoch in range(last_epoch + 1, args.num_epochs):
        ...
        if epoch % args.validation_epochs == 0 or epoch == args.num_epochs - 1:
            ...
            state = {'epoch': epoch,
                     ...
                     'scheduler': scheduler}
            net.save_checkpoint(state=state, model_path=model_path)
